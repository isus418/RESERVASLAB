...
function renderUserOptions() {
  ...
  if (remembered && LAB_USERS.includes(remembered)) {
    ...
    els.userSelect.classList.add("empty");
  }
}
...
function setUser(name) {
  ...
  els.userSelect.classList.toggle("empty", !name);
  renderGrid();
  updateMyCount();
}
...
// ---------- Init ----------
// Si todos los slots del día actual ya pasaron, saltar al día siguiente
// para no abrir la app con la grilla completamente gris.
const lastSlotToday = SLOTS[SLOTS.length - 1];
if (isPastSlot(currentDate, lastSlotToday)) {
